...
- `learn-claude-code`：学习载体。`20` 课 `harness` 工程教程，教你从零写 `agent loop`、工具、权限、记忆、多代理等全套机制。
- 企业 `RAG` 项目：应用目标。有前端菜单、文档导入、知识库、问答、检索、评估、日志、用户和角色模块，是学习成果最终要落地的地方。
...
| 优先级 | 项目 | 学习定位 | 怎么读 |
| --- | --- | --- | --- |
| `P0` | [`langchain-ai/retrieval-agent-template`](https://github.com/langchain-ai/retrieval-agent-template) | 唯一的 `RAG Agent` 主线，重点中的重点 | 完整读：索引图、检索图、用户隔离、上下文问答 |
| `P1` | [`langchain-ai/deployment-cookbook`](https://github.com/langchain-ai/deployment-cookbook) | 生产化：流式协议、线程历史、持久化、部署 | 只读运行时和持久化部分 |
| `P1` | [`openai/openai-agents-python`](https://github.com/openai/openai-agents-python) | `handoffs`、`tracing`、`sessions` 三个增量点 | 跳过 `tools` 和 `guardrails` 章节 |
| `P2` | [`pydantic/pydantic-ai`](https://github.com/pydantic/pydantic-ai) | 结构化输出、类型安全 | 读 `structured output` 文档即可 |
| `P2` | [`OpenHands/OpenHands`](https://github.com/OpenHands/OpenHands) | 真实复杂系统参考：沙箱、浏览器工具 | 只看架构文档和 `README`，不逐行读源码 |
| `P3` | [`langchain-ai/langgraph-101`](https://github.com/langchain-ai/langgraph-101) | `LangGraph` 视角对照（状态图、`checkpoint`） | 翻 `README` 对照即可，不跑环境 |

以下项目从清单中删除，理由：

- `langchain-ai/agents-from-scratch`：当前项目本身就是一套更完整的 `agents-from-scratch`（`20` 课从零写到综合），重复。仅当需要它的 `evaluation` 章节时单独查阅。
- `crewAIInc/crewAI`、`microsoft/autogen`：多代理编排框架。当前项目 `s15` 到 `s17` 的 `mailbox` 自组织模式已覆盖同类问题，且 `README` 的核心观点正是批判编排型 `agent`。仅作为框架选型对比时翻一下。
- `microsoft/agenticcookbook`：泛泛的示例集，与目标无关。

## 学习顺序（修订后）

原来四周的东西砍掉一半，只剩两条线：

### 第一条线：`RAG Agent`（对齐企业项目）
...
- `Agent` 状态如何持久化（对照 `s12` 的磁盘任务系统，看生产级做法）。
- 流式响应协议如何设计。
- 后端如何保存线程历史。
- `trace id`、耗时、`token`、错误类型如何追踪。

最小产出：

- 给企业项目补一份"生产化检查清单"。
- 设计一个 `Agent Run` / `Agent Step` 表结构草稿。
...
- 第 `1` 天：跑通 `retrieval-agent-template`，画出索引图和检索图。
- 第 `2` 天：研究用户隔离、权限组、`metadata` 过滤，对照企业项目权限模块写规则。
- 第 `3` 天：研究引用生成和上下文组装，写一页"企业 `RAG` 问答链路说明"。
- 第 `4` 天：读 `openai-agents-python` 的 `handoffs`、`tracing`、`sessions`。
- 第 `5` 天：读 `deployment-cookbook` 的流式协议和线程历史持久化。
- 第 `6` 天：设计 `Agent Run` / `Agent Step` 表结构 + 生产化检查清单。
- 第 `7` 天：复盘，翻一遍 `langgraph-101` 的 `state graph` 做对照，确定企业项目下一步改造优先级。
